...
class Memoryos:
    def __init__(self, user_id: str, 
        ...
        self.mid_term_memory = MidTermMemory(
            ...
            pinecone_index_name="memoryos-midterm"  # Use separate index for mid-term
        )
        self.user_long_term_memory = LongTermMemory(
            ...
            embedding_model_name=self.embedding_model_name,
            embedding_model_kwargs=self.embedding_model_kwargs,
            pinecone_api_key=os.environ.get("PINECONE_API_KEY"), # Pass Pinecone API key
            pinecone_environment=os.environ.get("PINECONE_ENVIRONMENT") # Pass Pinecone environment
        )
    ...
    def get_response(self, query: str, relationship_with_user="friend", style_hint="", user_conversation_meta_data: dict = None) -> str:
        ...
        # Ensure user_conversation_meta_data is a dictionary, defaulting to empty if None
        if user_conversation_meta_data is None:
            user_conversation_meta_data = {}
            
        # Ensure previous_conversation exists in meta_data with a default empty string
        if 'previous_conversation' not in user_conversation_meta_data:
            user_conversation_meta_data['previous_conversation'] = ""
