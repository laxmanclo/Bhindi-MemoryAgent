...
from pinecone import Pinecone  # For Pinecone vector search
...
class MidTermMemory:
    def __init__(self, file_path: str, client: OpenAIClient, max_capacity=2000, embedding_model_name: str = "all-MiniLM-L6-v2", 
                 embedding_model_kwargs: dict = None, pinecone_api_key: str = None, pinecone_environment: str = None,
                 pinecone_index_name: str = "memoryos-midterm"):
        ...
        # Embedding model configuration - needed before initializing Pinecone
        ...
        if self.pinecone_api_key and self.pinecone_environment:
            try:
                ...
                print(f"MidTermMemory: Initialized Pinecone client for environment: {self.pinecone_environment}")
                self._init_pinecone_index()
            except Exception as e:
                print(f"MidTermMemory: Error initializing Pinecone: {e}. Vector search may be limited.")
                raise e  # Require Pinecone to be available
                
    def _init_pinecone_index(self):
        """Initialize Pinecone index for mid-term memory sessions"""
        ...
        try:
            ...
            try:
                print(f"MidTermMemory: Connecting to existing Pinecone index '{self.pinecone_index_name}'...")
                ...
                print(f"MidTermMemory: Successfully connected to Pinecone index '{self.pinecone_index_name}' with dimension {self.dimension}.")
                self.pinecone_index = self.pinecone_client.Index(self.pinecone_index_name)
                return self.pinecone_index
            except Exception as e:
                if "NOT_FOUND" in str(e):
                    # Index doesn't exist, create it
                    print(f"MidTermMemory: Pinecone index '{self.pinecone_index_name}' not found. Creating new index...")
                    ...
                    print(f"MidTermMemory: Pinecone index '{self.pinecone_index_name}' created with dimension {self.dimension}.")
                    self.pinecone_index = self.pinecone_client.Index(self.pinecone_index_name)
                    return self.pinecone_index
                else:
            ...
            print(f"MidTermMemory: Error connecting to or creating Pinecone index '{self.pinecone_index_name}': {e}")
            raise e
            ...
            try:
                ...
                for i, sid in enumerate(session_ids):
                    vectors_to_upsert.append({
                        "id": sid,
                        "values": self.sessions[sid]["summary_embedding"],
